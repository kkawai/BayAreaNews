...
public final class HttpMessage {
	...
	public String post(final String name, final String value) throws Exception {
		...
		params.append(URLEncoder.encode(name, UTF_8)).append('=').append(URLEncoder.encode(value, UTF_8));
		return postData(params.toString().getBytes(UTF_8), "POST");
	}
	...
	private String postData(final byte[] data, final String httpMethod) throws Exception {
		...
		final BufferedReader rd = responseIsGzip(urlConnection) ? new BufferedReader(new InputStreamReader(new GZIPInputStream(urlConnection.getInputStream()),
				UTF_8), BUFFERED_READER_SIZE) : new BufferedReader(new InputStreamReader(urlConnection.getInputStream(), UTF_8), BUFFERED_READER_SIZE);
	...
	public String post(final Map<String, String> formParams) throws Exception {
		...
		if (formParams != null) {
			...
			for (final String key : formParams.keySet()) {
				params.append(URLEncoder.encode(key, UTF_8)).append('=').append(URLEncoder.encode(formParams.get(key), UTF_8)).append('&');
			}
	...
	public String delete() throws Exception {
		...
		final StringBuilder params = new StringBuilder();
		return postData(params.toString().getBytes(UTF_8), "DELETE");
	}
	...
	public String post(final List<NameValuePair> formParams) throws Exception {
		...
		if (formParams != null) {
			...
			for (final NameValuePair pair : formParams) {
				params.append(URLEncoder.encode(pair.getName(), UTF_8)).append('=').append(URLEncoder.encode(pair.getValue(), UTF_8)).append('&');
			}
			params.deleteCharAt(params.length() - 1);
		}
		return postData(params.toString().getBytes(UTF_8), "POST");
	...
	private String getString(final InputStream is) throws Exception {
		...
		while ((read = is.read(buf)) != -1) {
			sb.append(new String(buf, 0, read, UTF_8));
		}
		return sb.toString();
	}
